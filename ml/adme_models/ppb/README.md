...
Had to use quite a few keyword tricks to find the right data and ha to use some multiprocessing to avoid angering the Chembl REST gods. There was actually lots of non-human data (rat and mouse) and the species and the readouts/units (%) had to be inferred from the `assay_description` to get enough data points. Given the amount of rat data (more than human), this is probably a decent test case for some transfer learning or MTL if the model peformance wasn't decent. See `2025-10-08_retrieve_chembl_ppb_f_u.ipynb` for code and further details re: post-processing clean-up of the data.
