...
def retrieve_chembl_bioactivities(uniprot_id: str, readout: str, relation:str, assay_type: str) -> pd.DataFrame:
    ...
    bioactivities = bioactivities_api.filter(
        target_chembl_id=chembl_id, type=readout, relation=relation, assay_type=assay_type
    ).only(
        "activity_id",
        "assay_chembl_id",
        "assay_description",
...
def retrieve_logd_values(logd_readouts, relation:str, number_of_records:str) -> List:
    
    results = []

    # To ensure we get as many LogD's as possible, run the query for multiple LogD readouts. 

    logd_readouts = logd_readouts

    # Get logD activities
    for logd_readout in logd_readouts:
        activities = new_client.activity.filter(standard_type=logd_readout, relation=relation).only(
            "activity_id",
            "assay_chembl_id",
            "molecule_chembl_id",
            "standard_type",
        ...
        print(f'{logd_readout}: len({activities} records)')

        for act in activities[number_of_records]: 
            record = {
            ...
            # Get assay description and type
