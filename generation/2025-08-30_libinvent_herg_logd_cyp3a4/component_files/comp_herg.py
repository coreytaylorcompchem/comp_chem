...
class Herg:
    ...
    def __call__(self, smiles: List[str]) -> SmilesAssociatedComponentResults:
        scores = []
        for smi in smiles:
            data = mol_to_graph(smi, label=0)
            if data is None:
                logger.warning(f"Invalid SMILES: {smi}")
                scores.append(float('nan'))  # Use NaN for failures as per docs
                continue
            data = data.to(self.device)
            with torch.no_grad():
                raw_output = self.model(data).item()
                score = float(torch.sigmoid(torch.tensor(raw_output)).item())
            logger.info(f"SMILES: {smi} | Score: {score} | Type: {type(score)}")
            scores.append(score)

        # Wrap scores into ComponentResults
        component_results = ComponentResults(scores=[np.array(scores)])
